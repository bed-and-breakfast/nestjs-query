...
class AggregateBuilder {
    ...
    createAggSelect(func, aggregatedFields) {
        if (!aggregatedFields) {
            return {};
        }
        return aggregatedFields.reduce((agg, { field }) => {
    ...
    createGroupBySelect(aggregatedFields) {
        if (!aggregatedFields) {
            return null;
        }
        return aggregatedFields.reduce((id, aggregatedField) => {
            const aggAlias = this.getGroupByAlias(aggregatedField);
            const fieldAlias = `$${(0, helpers_1.getSchemaKey)(String(aggregatedField.field))}`;
            return { ...id, [aggAlias]: fieldAlias };
        }, {});
    }
    getGroupBySelects(aggregatedFields) {
        if (!aggregatedFields) {
            return undefined;
        }
